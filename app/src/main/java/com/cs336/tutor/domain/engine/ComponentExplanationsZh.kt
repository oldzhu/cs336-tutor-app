...
object ComponentExplanationsZh {
    ...
    val embedding = mapOf(
        ...
        8 to "前向传播：x 为 token 索引，返回嵌入。",
        9 to "按 token ID 查找嵌入向量。梯度通过被选中行反向传播。"
    )
    ...
    val lmhead = mapOf(
        ...
        8 to "前向传播：每位置隐藏状态投影到词汇表 logits。",
        9 to "输出 (batch,seq,vocab_size)。最高分即预测下一个 token。"
    )
    ...
    val optimizer = mapOf(
        ...
        9 to "第二矩（方差）：为每个参数自适应调整学习率。",
        10 to "参数更新：动量/√方差。每个参数独立学习率。"
    )
    ...
    val training = mapOf(
        ...
        10 to "关键行：标准语言建模交叉熵损失。",
        11 to "展平 logits 为 (batch*seq,vocab_size)。",
        15 to "反向传播：自动微分计算所有参数梯度。",
        16 to "应用梯度：优化器规则更新所有参数。"
    )
